...
local function handle_aa(type,offset)
    ...
    if(not local_pl or not entity_is_alive(local_pl)) then return end
    ...
    local lp_vel = get_velocity(local_pl)
    ...
    if type == 1 then 
        ...
        if not isFreestanding then 
            ui_set(aa_yaw_offset, mode == 2 and 0)
            ui_set(aa_body_yaw, "static")
            ui_set(aa_body_yaw_slider, -180)
        else
            ui_set(aa_yaw_offset, (mode == 2 and crouching_ct and -13) or 0)
            ui_set(aa_body_yaw, "static")
            if anti_brute_FORCE then offset = -enemy_shot_angle[enemyclosesttocrosshair] end
            ui_set(aa_body_yaw_slider, (desync_onshot and -offset) or offset)	
        end

        if type == 2 and crouching_ct then
...
local function on_paint(c)	
    ...
    if isFreestanding then
		if (no_angle and dangerous_x_offset and dangerous_y_offset and not flip_angle and not in_air(entity_get_local_player()) and get_velocity(entity_get_local_player()) < 130) then
            handle_aa(1,0)
		...
		elseif direct_mode == "Dracos" then
			if adaptive_freestand < 0 then
				if enemy_is_peeking_and_can_hit_us(enemyclosesttocrosshair) and (adjusted_freestand < globals_curtime()) then
                    ...
                    handle_aa(2,flipJitter and -2 or 0)
                    client.draw_text(c, center_x - 60, center_y, 163,160,163,255, "cb+", 0, "⯇")
                    client.draw_text(c, center_x + 60, center_y, 163,160,163,255, "cb+", 0, "⯈")
				else
...
client_set_event_callback("run_command", function (e)
    ...
    -- EDGE YAW
	if ui_get(edge_yaw_detection) and not can_enemy_hit_head(enemyclosesttocrosshair) and not jumping and not ui_get(misc_fakeduck_key) then
	...
	if ui_get(leg_movement) then
		if hit then
			...
			ui_set(misc_leg_movement,flipJitter and "always slide" or "never slide")
		end	
	else
